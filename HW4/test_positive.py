...
class TestPositive:
    def save_log(self, start_time, file_name):
    ...
    def test_step2(self, clear_folders, make_files, start_time):
        res = [ssh_checkout(data["host"], data["user"], data["passwd"],
                            ...
                            "Everything is Ok"), ssh_checkout(data["host"], data["user"], data["passwd"],
                                                              f"cd {data['FOLDER_OUT']}; 7z e arx2.{data['extension']}"
                                                              f" -o{data['FOLDER_EXTRACT']} -y",
                                                              "Everything is Ok")]
        for item in make_files:
            res.append(ssh_checkout(data["host"], data["user"], data["passwd"], f"ls {data['FOLDER_EXTRACT']}", item))
    ...
    def test_step3(self, start_time):
        ...
        assert ssh_checkout(data["host"], data["user"], data["passwd"],
                            f"cd {data['FOLDER_OUT']}; 7z t arx2.{data['extension']}", "Everything is Ok"), "test3 FAIL"

    def test_step4(self, start_time):
        self.save_log(start_time, 'log4.txt')
        assert ssh_checkout(data["host"], data["user"], data["passwd"], f"cd {data['FOLDER_OUT']}; 7z d arx2.7z",
                            "Everything is Ok"), "test4 FAIL"

    def test_step5(self, start_time):
        self.save_log(start_time, 'log5.txt')
        assert ssh_checkout(data["host"], data["user"], data["passwd"], f"cd {data['FOLDER_OUT']}; 7z u arx2.7z",
    ...
    def test_step6(self, clear_folders, make_files, start_time):
        ...
        for item in make_files:
            res.append(ssh_checkout(data["host"], data["user"], data["passwd"],
                                    f"cd {data['FOLDER_OUT']}; 7z l arx2.{data['extension']}", item))
    ...
    def test_step7(self, clear_folders, make_files, make_subfolder, start_time):
        res = [ssh_checkout(data["host"], data["user"], data["passwd"],
                            ...
                            "Everything is Ok"), ssh_checkout(data["host"], data["user"], data["passwd"],
                                                              f"cd {data['FOLDER_OUT']}; 7z x arx.{data['extension']} "
                                                              f"-o{data['FOLDER_EXTRACT2']} -y",
                                                              "Everything is Ok")]
        for item in make_files:
            res.append(ssh_checkout(data["host"], data["user"], data["passwd"], f"ls {data['FOLDER_EXTRACT2']}", item))
        res.append(ssh_checkout(data["host"], data["user"], data["passwd"], f"ls {data['FOLDER_EXTRACT2']}",
                                make_subfolder[0]))
        res.append(ssh_checkout(data["host"], data["user"], data["passwd"],
                                f"ls {data['FOLDER_EXTRACT2']}/{make_subfolder[0]}", make_subfolder[1]))
    ...
    def test_step8(self, clear_folders, make_files, start_time):
        ...
        for item in make_files:
            ...
            res.append(
                ssh_checkout(data["host"], data["user"], data["passwd"], f"cd {data['FOLDER_IN']}; 7z h {item}", hash))
    ...
    def test_step99(self):
        res = [ssh_checkout(data["host"], data["user"], data["passwd"],
                            ...
                            "Removing"), ssh_checkout(data["host"], data["user"], data["passwd"],
                                                      f"echo {data['passwd']} | sudo -S dpkg -s p7zip-full",
                                                      "Status: deinstall ok")]
