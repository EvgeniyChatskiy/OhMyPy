...
def make_folders():
    return ssh_checkout(data["host"], data["user"], data["passwd"],
                        f"mkdir {data['FOLDER_IN']} {data['FOLDER_OUT']} {data['FOLDER_EXTRACT']} "
...
def make_files():
    ...
    for i in range(data['count']):
        ...
        if ssh_checkout(data["host"], data["user"], data["passwd"],
                        f"cd {data['FOLDER_IN']}; dd if=/dev/urandom of={filename} bs={data['size']} "
...
def make_subfolder():
    ...
    if not ssh_checkout(data["host"], data["user"], data["passwd"], f"cd {data['FOLDER_IN']}/{subfoldername}; "
                                                                    f"dd if=/dev/urandom of={testfilename} "
                                                                    f"bs={data['size']} "
                                                                    f"count=1 iflag=fullblock",
                        ""):
...
def make_bad_file():
    if ssh_checkout(data["host"], data["user"], data["passwd"],
            ...
            ssh_checkout(data["host"], data["user"], data["passwd"], f"truncate -s 1 {data['FOLDER_OUT']}/arx2_bad.7z",
                         ""):
        return 'arx2_bad'
    return None
