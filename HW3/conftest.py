...
def make_files():
    ...
    for i in range(data['count']):
        ...
        if checkout(f"cd {data['FOLDER_IN']}; dd if=/dev/urandom of={filename} bs={data['size']} "
...
def make_subfolder():
    ...
    if not checkout(f"cd {data['FOLDER_IN']}; mkdir {subfoldername}", ""):
        return None, None
    if not checkout(
            f"cd {data['FOLDER_IN']}/{subfoldername}; dd if=/dev/urandom of={testfilename} bs={data['size']} "
            f"count=1 iflag=fullblock",
            ""):
