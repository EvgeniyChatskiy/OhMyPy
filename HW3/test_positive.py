...
class TestPositive:
    ...
    def test_step1(self, make_folders, clear_folders, make_files):
        res1 = checkout(f"cd {data['FOLDER_IN']}; 7z a -t{data['extension']} {data['FOLDER_OUT']}/arx2",
                        "Everything is Ok")
        res2 = checkout(f"ls {data['FOLDER_OUT']}", f"arx2.{data['extension']}")
        assert res1 and res2, "test1 FAIL"

    def test_step2(self, clear_folders, make_files):
        res = [checkout(f"cd {data['FOLDER_IN']}; 7z a -t{data['extension']} {data['FOLDER_OUT']}/arx2",
                        "Everything is Ok"),
               checkout(f"cd {data['FOLDER_OUT']}; 7z e arx2.{data['extension']} -o{data['FOLDER_EXTRACT']} -y",
                        "Everything is Ok")]
    ...
    def test_step7(self, clear_folders, make_files, make_subfolder):
        res = [checkout(f"cd {data['FOLDER_IN']}; 7z a -t{data['extension']} {data['FOLDER_OUT']}/arx",
                        "Everything is Ok"),
               checkout(f"cd {data['FOLDER_OUT']}; 7z x arx.{data['extension']} -o{data['FOLDER_EXTRACT2']} -y",
                        "Everything is Ok")]
        for item in make_files:
